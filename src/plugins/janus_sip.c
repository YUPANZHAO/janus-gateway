...
static void janus_sip_hangup_media_internal(janus_plugin_session *handle) {
	...
	janus_mutex_lock(&session->mutex);
	if(session->stack && session->stack->s_nh_i != NULL && session->callee != NULL) {
		g_free(session->callee);
		session->callee = NULL;
		janus_mutex_unlock(&session->mutex);
		/* Send a BYE */
	...
	while(temp) {
		...
		if(m->type == JANUS_SDP_AUDIO || m->type == JANUS_SDP_VIDEO || !strcasecmp(m->proto, "UDP/BFCP")) {
			...
			if(m->port) {
				...
				if(update && (!session->media.mlines[m->index].remote_ip || !m->c_addr ||
				...
				if(m->c_addr == NULL) {
					session->media.mlines[m->index].remote_ip = g_strdup(sdp->c_addr);
				}else {
