...
bfcp_message *bfcp_build_message(bfcp_arguments* arguments)
{
    ...
    switch(arguments->primitive) {
        ...
        case e_primitive_FloorRequestStatusAck:
            return bfcp_build_message_FloorRequestStatusAck(arguments->entity, arguments->fID);
        case e_primitive_FloorStatusAck:
            return bfcp_build_message_FloorStatusAck(arguments->entity, arguments->fID);
		default:
...
bfcp_message *bfcp_build_message_FloorRequestStatusAck(bfcp_entity *entity, bfcp_floor_id_list *fID)
{
    int err = 0;
	bfcp_message *message = bfcp_new_message(NULL, 0);
	if(!message)	/* We could not allocate the memory, return a with failure */
    ...
    // add by csk, add floor ID in this msg
    bfcp_floor_id_list *temp = fID;
    while(temp) {	/* There can be more than one FLOOR-ID attribute */
        if(!temp->ID)	/* We need a valid FloorID */ {
            return NULL;}
...
bfcp_message *bfcp_build_message_FloorStatusAck(bfcp_entity *entity, bfcp_floor_id_list *fID)
{
	int err;
	bfcp_message *message = bfcp_new_message(NULL, 0);
	if(!message)	/* We could not allocate the memory, return a with failure */
...
int bfcp_build_attribute_OVERALL_REQUEST_STATUS(bfcp_message *message, bfcp_overall_request_status *oRS)
{
    if(!oRS)	/* There's no Overall Request Status, return wih a failure */
        ...
        if (oRS->fRS) {
            err = bfcp_build_attribute_FLOOR_REQUEST_STATUS(message, oRS->fRS);
            if(err == -1)	/* We couldn't build this attribute, return with an error */
                return -1;
            attrlen = attrlen+err;
        }
