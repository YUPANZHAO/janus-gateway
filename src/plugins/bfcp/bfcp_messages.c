...
bfcp_floor_request_status *bfcp_new_floor_request_status(UINT16 fID, bfcp_request_status* rs, char *sInfo)
{
    ...
    if (rs) { // modify by csk, optional argument
        floor_request_status->rs = bfcp_new_request_status(rs->rs, rs->qp);
...
bfcp_overall_request_status *bfcp_new_overall_request_status(UINT16 frqID, UINT16 rs, UINT16 qp, bfcp_floor_request_status* fRS, char *sInfo)
{
	...
	if(!overall_request_status->rs)
		return NULL;
    if (fRS) {
        overall_request_status->fRS = bfcp_new_floor_request_status(fRS->fID, fRS->rs, fRS->sInfo); // add by csk
    }
	if(sInfo) {
...
int bfcp_free_overall_request_status(bfcp_overall_request_status *overall_request_status)
{
    ...
    if (overall_request_status->fRS)  // add by csk
        res += bfcp_free_floor_request_status_list(overall_request_status->fRS);
	if(overall_request_status->sInfo)
